...
fn _alphabeta_poll(game: &Game, remaining_depth: usize, alpha: f64, beta: f64, score_map: &BTreeMap<Vec<u8>, f64>) -> Result<f64, Vec<Vec<u8>>> {
    if remaining_depth <= 0 {
    ...
    if game.is_p1_moving_next() {
        ...
        for next_state in next_states {
            value = value.max(_alphabeta_poll(&next_state, remaining_depth - 1, value, beta, score_map)?);
            if value >= beta {
                break
            }
        }
        ...
        for next_state in next_states {
            value = value.min(_alphabeta_poll(&next_state, remaining_depth - 1, alpha, value, score_map)?);
            if value <= alpha {
                break
            }
        }
...
pub fn alphabeta_poll(game: &Game, depth: usize, score_map: &BTreeMap<Vec<u8>, f64>) -> Result<(Game, Action), Vec<Vec<u8>>> {
    let (next_states, actions) = game.expand(true);

    let mut zipped: Vec<_> = next_states.into_iter().zip(actions.into_iter()).collect();
    random_shuffle(&mut zipped);

    let values = zipped.iter().map(|(next_state, _)| _alphabeta_poll(next_state, depth - 1, std::f64::NEG_INFINITY, std::f64::INFINITY, score_map)).collect::<Vec<_>>();
    let i = if game.is_p1_moving_next() {
        values.iter().enumerate().max_by(|(_, a), (_, b)| a.partial_cmp(b).unwrap()).unwrap().0
    } else {
        values.iter().enumerate().min_by(|(_, a), (_, b)| a.partial_cmp(b).unwrap()).unwrap().0
